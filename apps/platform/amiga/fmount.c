...
static int drive_to_unit(const char *s) {
  ...
  if ((s[0]=='D'||s[0]=='d') && (s[1]=='N'||s[1]=='n') && s[2]>='0' && s[2]<='7' &&
      (s[3]==':' || s[3]=='\0')) return s[2] - '0';
  return -1;
}
static int is_ro(const char *s) { return s && (s[0]=='R'||s[0]=='r') && (s[1]=='O'||s[1]=='o') && !s[2]; }
static int is_rw(const char *s) { return s && (s[0]=='R'||s[0]=='r') && (s[1]=='W'||s[1]=='w') && !s[2]; }

static void snapshot_node(int unit, node_snapshot_t *out)
{
  char name[4]; struct DosList *list, *entry; struct FileSysStartupMsg *startup;
  struct DosEnvec *env;
  memset(out, 0, sizeof(*out)); sprintf(name, "DN%d", unit);
  list = LockDosList(LDF_READ | LDF_DEVICES);
  if (!list) return;
  entry = FindDosEntry(list, (CONST_STRPTR)name, LDF_DEVICES);
  if (entry) {
    out->present = TRUE; out->task = entry->dol_Task; out->active = out->task != NULL;
    startup = entry->dol_misc.dol_handler.dol_Startup ?
      (struct FileSysStartupMsg *)BADDR(entry->dol_misc.dol_handler.dol_Startup) : NULL;
    env = startup && startup->fssm_Environ ? (struct DosEnvec *)BADDR(startup->fssm_Environ) : NULL;
    if (env) { out->bpt=env->de_BlocksPerTrack; out->surfaces=env->de_Surfaces;
      out->low=env->de_LowCyl; out->high=env->de_HighCyl; out->dostype=env->de_DosType; }
  }
                                 ...
                                 uint32_t dostype)
{
  char name[4]; struct DosList *list, *entry; struct FileSysStartupMsg *startup;
  struct DosEnvec *env;
  sprintf(name, "DN%d", unit); list = LockDosList(LDF_READ | LDF_DEVICES);
  if (!list) return 30;
  entry = FindDosEntry(list, (CONST_STRPTR)name, LDF_DEVICES);
  startup = entry && entry->dol_misc.dol_handler.dol_Startup ?
    (struct FileSysStartupMsg *)BADDR(entry->dol_misc.dol_handler.dol_Startup) : NULL;
  env = startup && startup->fssm_Environ ? (struct DosEnvec *)BADDR(startup->fssm_Environ) : NULL;
  if (!entry || entry->dol_Task || !env) { UnLockDosList(LDF_READ | LDF_DEVICES); return 30; }
  env->de_Surfaces = profile->surfaces; env->de_BlocksPerTrack = profile->blocks_per_track;
  env->de_LowCyl = profile->low_cylinder; env->de_HighCyl = profile->high_cylinder;
...
int main(int argc, char **argv)
{
  ...
  struct MsgPort *port; struct IOExtTD *request; struct fujinet_disk_catalog_mount catalog;
  struct fujinet_disk_catalog_inspection inspection; fujinet_disk_media_profile_t profile;
  node_snapshot_t node; uint32_t dostype; BOOL compatible, inhibited = FALSE; LONG result, uninhibit;
  if (argc < 2 || argc > 4 || argv[1][0]=='?') { usage(); return 10; }
  for (i=0; argv[1][i]>='0' && argv[1][i]<='9'; ++i) slot_val=slot_val*10+argv[1][i]-'0';
  if (argv[1][i] || slot_val > 255) { puts("Bad slot"); return 10; }
  slot=(uint8_t)slot_val; unit=(int)slot;
  for (i=2;i<argc;++i) { int parsed=drive_to_unit(argv[i]);
    if (parsed>=0) unit=parsed; else if (is_ro(argv[i])) readonly=1; else if (is_rw(argv[i])) readonly=0;
    else { usage(); return 10; } }
  if (unit<0 || unit>FUJINET_DISK_MAX_UNIT) { puts("Bad drive"); return 10; }
  port=CreatePort(NULL,0); request=port?(struct IOExtTD *)CreateExtIO(port,sizeof(*request)):NULL;
  if (!request || OpenDevice((CONST_STRPTR)FUJINET_DISK_DEVICE_NAME,(ULONG)unit,(struct IORequest *)request,0)!=0) {
    fprintf(stderr,"FMOUNT OPEN_DEVICE failed request=%p error=%ld\n",(void *)request,
            request ? (long)request->iotd_Req.io_Error : (long)IoErr()); return 20; }
  memset(&inspection,0,sizeof(inspection)); inspection.catalog_slot=slot;
  request->iotd_Req.io_Command=FUJINET_DISK_CMD_INSPECT_CATALOG;
  request->iotd_Req.io_Data=&inspection; request->iotd_Req.io_Length=sizeof(inspection);
  ...
  if (result!=0 || fujinet_disk_classify_media_profile(&inspection.inspection.media,&profile)!=FN_OK ||
    ...
    fprintf(stderr,"FMOUNT INSPECT failed io=%ld profile=%u filesystem=%u\n",(long)result,
            (unsigned)fujinet_disk_classify_media_profile(&inspection.inspection.media,&profile),
            (unsigned)fujinet_disk_classify_filesystem(inspection.inspection.boot_bytes,inspection.inspection.boot_length,&dostype)); goto fail; }
  ...
  if (compatible && node.active) {
    if (!Inhibit((CONST_STRPTR)dos_name,DOSTRUE)) { fprintf(stderr,"Cannot inhibit %s\n",dos_name); goto fail; }
    inhibited=TRUE;
  } else if (!compatible && retire_handler(unit,&node)!=0) goto fail;
  catalog.catalog_slot=slot; catalog.writable=readonly?0:1;
